(function () {
  ...
  function waitForHidden(modalEl) {
  ...
  function cleanupModalBackdropsIfNoOpenModals() {
    try {
      ...
      console.warn('cleanupModalBackdropsIfNoOpenModals error', e);
    }
  }
  ...
  function cleanupModalBackdrops() {
    try {
      ...
      console.warn('cleanupModalBackdrops error', e);
    }
  }
  ...
  function showPreviewModal(project) {
    ...
    if (!modalEl) {
      console.warn('Preview modal no encontrado');
      return;
    }
    ...
    const otherModal = $('enterProjectModal') || null;
    const otherInst = otherModal ? bootstrap.Modal.getInstance(otherModal) : null;

    (async () => {
      try {
        if (otherInst) {
          otherInst.hide();
          await waitForHidden(otherModal);
        }
            ...
            async () => {
              ...
              try {
                ...
                document.dispatchEvent(
                  new CustomEvent('proyekta:project-selected', { detail: project }),
                );
                ...
                try {
                  ...
                  console.warn('Limpieza defensiva falló', e);
                }
        ...
        const previewInst = bootstrap.Modal.getInstance(modalEl) || new bootstrap.Modal(modalEl);
        previewInst.show();
      } catch (err) {
        console.error('Error en showPreviewModal:', err);
        try {
          if (document.querySelectorAll('.modal.show').length === 0) {
            document.querySelectorAll('.modal-backdrop').forEach((n) => n.remove());
            document.body.classList.remove('modal-open');
      ...
      <div class="p-3">
        <div class="d-flex align-items-start justify-content-between mb-3">
          <div>
            ...
            <div class="small text-muted text-white-50">${escapeHtml(
              project.description || '',
            )}</div>
  ...
  async function onBtnEnterProjectClick() {
    ...
    if (!idVal) {
      if (enterModalEl) {
        ...
        if (footer) {
          const existing = document.getElementById('enterProjectWarning');
          ...
          setTimeout(() => {
            const el = document.getElementById('enterProjectWarning');
            if (el) el.remove();
          }, 2500);
        }
      }
